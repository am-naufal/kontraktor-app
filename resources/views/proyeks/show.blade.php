...
<div class="container">
    ...
    <li class="d-flex justify-content-between align-items-center">
        ...
        <form action="{{ route('proyeks.removeCustomer', [$proyek->id, $customer->id]) }}" method="POST" onsubmit="return confirm('Yakin hapus customer ini dari proyek?')" class="ml-3">
            @csrf
            @method('DELETE')
...
@endforeach
                ...
                <dd class="col-sm-9">
                    <form action="{{ route('proyeks.updateStatus', $proyek->id) }}" method="POST" class="form-inline d-flex align-items-center gap-2">
                        @csrf
                        @method('PUT')
    ...
    {{-- Form Tambah Customer ke Proyek --}}
    <div class="card">
        <div class="card-header">
            Tambah Customer ke Proyek
        </div>
        <div class="card-body">
            <form action="{{ route('proyeks.addCustomer', $proyek->id) }}" method="POST">
                @csrf
                <div class="form-group">
                    ...
                    <select name="customer_id" class="form-control" required>
                        ...
                        @foreach ($availableCustomers as $customer)
                            <option value="{{ $customer->id }}">{{ $customer->nama }} - {{ $customer->nama_perusahaan }}</option>
                        @endforeach
                    </select>
                </div>
